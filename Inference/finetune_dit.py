...
def stage1(diffusion, model, vae, loader, device):
    xts=list()
    for x, ry, y in loader:
        ...
        t = torch.ones((1,), dtype=torch.long, device=device) * (diffusion.num_timesteps - 1)
        y = torch.tensor([y], device=device)
        model_kwargs = dict(y=y.to(device))
        x_t = diffusion.ddim_reverse_sample(model, latents.to(device), t, model_kwargs=model_kwargs, clip_denoised=True)
        xts.append(x_t)
    return xts
