...
def main(args):
    ...
    logger.info(f"Dataset contains {len(combined_dataset):,} images ({args.data_path})")
    ...
    checkpoint = torch.load(f"path/to/Folder/Train/fusion_module_with_text/{args.dataset}-{args.spec}-{args.image_size}x{args.image_size}.pth", map_location=f"cuda:{device}", weights_only=True)
    ...
    for epoch in range(args.epochs):
        ...
        for x1, ry, y, x2, texts, labels in combined_loader:
            assert torch.equal(ry, labels)
            ry = ry.numpy()
            x1 = x1.to(device)
            y = y.to(device)
            x2 = x2.to(device)
            ...
            with torch.no_grad():
                ...
                z_fused = fusion_module(x1, text_embeds)
                x = z_fused * 0.18215
