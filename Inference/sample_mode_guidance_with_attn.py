...
import argparse
from tsne_plots import define_model, get_loader, get_loader_with_attn, get_features_per_class_with_attn
...
def attn_dataloader(orig_path, batch_size, shuffle=False):
    transform = transforms.Compose([
        transforms.Lambda(lambda pil_image: center_crop_arr(pil_image, image_size=512)),
...
def main(args):
    ...
    elif args.spec == 'imagenet-1k':
        file_list = './misc/class_imagenet-1k.txt'
    else:
    ...
    if args.cluster_file is None:
        ...
        fusion_module = CrossAttentionFusion(num_layers=2, image_size=args.image_size).to(device)
        checkpoint = torch.load(f"../Train/fusion_module_with_text/{args.dataset}-{args.spec}-{args.image_size}x{args.image_size}.pth", map_location=device, weights_only=True)
        state_dict = checkpoint["fusion_module"] if "fusion_module" in checkpoint else checkpoint
        new_state_dict = {
            k.replace("module.", ""): v for k, v in state_dict.items()
        }
        fusion_module.load_state_dict(new_state_dict)
        fusion_module.eval()
        for p in fusion_module.parameters():
            p.requires_grad = False

        original_loader = get_loader_with_attn(args, return_path=True)
        original_features_per_class, original_paths = get_features_per_class_with_attn(args, original_loader, vae, fusion_module, text_encoder, tokenizer)
